...
const SignUp = (props) => {
    ...
    const validate = () => {
        ...
        if (formData.username.trim().length < 4) {
            ...
            validationErrors.username = true;
            setErrors((prevErrors) => {
                return { ...prevErrors, username: `Username should'n have empty characters` };
            });
        } else {
        ...
        if (formData.password.trim().length < 6) {
            ...
            setErrors((prevErrors) => {
                return { ...prevErrors, password: 'Password should have at least 4 characters' };
            });
        } else if (!/^[^\s]*$/.test(formData.password.trim())) {
            validationErrors.password = true;
            setErrors((prevErrors) => {
                return { ...prevErrors, password: 'Password should n have empty characters' };
            });
        ...
        if (formData.password.trim() !== formData.confirmPassword.trim()) {
            ...
            setErrors((prevErrors) => {
                return { ...prevErrors, confirmPassword: 'Password should be the same' };
            });
        } else {
    ...
    return (
        <div className="signUp cos">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <input type="password" name="password" placeholder="Confirm password" onChange={handleInputChange} />
